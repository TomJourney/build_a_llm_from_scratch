...
### 【6.1.1】如何把模型输出转换为类别标签预测

【回顾： 生成下一个词元id的方法】 

DiyGPTModel预测输入文本的下一个词元时，输出该文本的下一个词元的概率（共计有50257个词元，每个词元都有一个概率，所以共计50257个概率），然后返回最高概率的位置，该位置就是大模型生成的下一个词元的词元id；

<font color=red>【类别标签预测的方法】</font>

还是计算最高概率的位置（下标），只不过输出是2个词元的概率（字典只有2个值0与1）；简单理解下标=0表示非垃圾消息，下标=1表示垃圾消息；

---

【test0605_p168_add_classification_head_main.py】 测试案例-新增分类头（计算最高概率的位置）
